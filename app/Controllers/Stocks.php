...
class Stocks extends BaseController{
    ...
    public function familias(){
        ...
        $data['familias'] = $model->get_all_families(); // familias referencia de variável na view - é o padrão        
    ...
    public function familia_adicionar(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            // verificar se já existe a família com mesmo nome (designacao)
            ...
            if($error == ''){
                $model->family_add();
                $data['success'] = "Família adicionada com sucesso."; // success variável na view 
                $data['familias'] = $model->get_all_families(); // atualiza os dados para vizualização 
            } else {
                $data['error'] = $error;
            }
        }
    ...
    public function familia_editar($id_familia){
        ...
        // carregar os dados das famílias para passar para a view
        $model = new StocksModel();
        $data['familias'] = $model->get_all_families(); // todas famílias 
        $data['familia'] = $model->get_family($id_familia); // familia especifica passando para o editar! 
        $error = '';
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            // verificar se já existe a família com mesmo nome // evitando repetição 
    ...
    public function familia_eliminar($id_familia, $resposta = 'nao'){ 
        ...
        if($resposta == 'sim'){
            ...
            $model->delete_family($id_familia); /// elimina a familia alterando o id_parent para 0 das ligações se possuir 
    ...
    public function produtos_adicionar(){
        ...
        // tratar a submissao do formulario de adição de produto
        if($_SERVER['REQUEST_METHOD'] == 'POST'){ // POST manda as informações do formulário.

            // definição do nome da imagem do produto // microtime = milisegundos 
            $novo_ficheiro = round(microtime(true) * 1000) . '.'.pathinfo($_FILES["file_imagem"]["name"], PATHINFO_EXTENSION);
            // posso concatenar ainda com '.'. unicid()
    ...
    // ==================================================
    public function produtos_editar($id){ // Apresentando produtos e fazendo a alteração de dados!

        helper('funcoes');
        $id = aesDecrypt($id); // desencriptando dentro do helpers // id vindo da view produtos_editar
        if($id == -1) { return; } // -1 devolveu erro 
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){                   
            ...
            if($erro == ''){
                ...
                if( !file_exists($_FILES['file_imagem']['tmp_name']) || 
                    !is_uploaded_file($_FILES['file_imagem']['tmp_name'])){//se não foi ficheiro temp carregado
                    $existe_ficheiro_para_upload = false;
                }

                // verifica se é necessário carregar novo ficheiro // caso tenha 
                if($existe_ficheiro_para_upload){ // se tiver ficheiro faz o UP
        ...
        // buscar os dados do produto a editar        
        $result = $model->get_product($id); // produto específico 
    ...
    public function produtos_eliminar($id_produto, $resposta = 'nao'){
        ...
        helper('funcoes');
        $id_produto = aesDecrypt($id_produto); // desencriptação do $id_produto
    ...
    public function taxas_adicionar(){
        ...
        $model = new StocksModel();
        $data = array(); // para evitar erro 
        $error = '';

        // adicionar nova taxa //  a taxa existe por sí mesma 
        if($_SERVER['REQUEST_METHOD'] == 'POST'){            
