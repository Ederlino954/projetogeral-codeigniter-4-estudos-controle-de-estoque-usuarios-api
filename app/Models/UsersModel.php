...
use CodeIgniter\Model;  /// model incorporado dentro do CodeIgniter
...
class UsersModel extends Model
{
    ...
    public function __construct(){
        $this->db = db_connect(); // padrão de conexão da documentaçao 
    }

    // =============================================
    public function verifyLogin($username, $password){
        $params = array( // evitando sql injection 
            $username,
            md5(sha1($password))
        );

        $query = "SELECT * FROM users WHERE username = ? AND passwrd = ? AND deleted = 0"; // deleted inativo ou não para login 
        $results = $this->db->query($query,$params)->getResult('array'); // Array assossiativo 
        ...
        if(count($results) == 0){
            ...
            // returns valid login
            return $results[0]; // devolvendo uma linha com [0]
        }
    }
    ...
    public function resetPassword($email){ 
        ...
        if(count($results) != 0){ // id está no results 
            // existe o email

            // change the user's password
            $newPassword = $this->randomPassword(); // password aleatória 
            $params = array(
    ...
    public function redefinePassword($id, $pass){
        ...
        // removes the purl from the user // atualizando purl, deixando vazio 
        $params = array(
            $id
        );
    ...
    // =============================================
    public function getUser($id_user){ // usuário específico 
    ...
    // =============================================
    public function checkExistingUser(){ // checando username e emails repetidos 
    ...
    public function checkAnotherUserEmail($id_user){
        ...
        // verifies if there is already another user with the same email address // verificando a repetição de email 
        $request = \Config\Services::request();
        $dados = $request->getPost();
        ...
        return $this->db->query('SELECT id_user FROM users WHERE email = ? AND id_user <> ?', $params)->getResult('array'); // diferente        
    }
    ...
    public function addNewUser(){
        ...
        if(isset($dados['check_admin'])){
            array_push($profileTemp, 'admin'); // PUSH POSICIONANDO AO FUNDO
        }
        ...
        $profile = implode(',',$profileTemp); // juntando array com virgula 
    ...
    // =============================================
    public function deleteUser($id_user){ // Deixa Inativo 
    ...
    // =============================================
    public function recoverUser($id_user){ // Deixas Ativo
    ...
    // =============================================
    private function randomPassword($numChars = 8){ // password aleatório 8 caracteres 
