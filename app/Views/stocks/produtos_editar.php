<?php 
    ...
    helper('funcoes');
    $id = aesEncrypt($produto['id_produto']); // sendo enviado ao controller stock
?>
        ...
        <form action="<?php echo site_url('stocks/produtos_editar/'.$id)?>" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="combo_familia" class="form-control">
                    ...
                    <?php if($produto['id_familia'] == 0): ?> <!-- Não existencia familia -->
                        <option value="0" selected>Nenhuma</option>
                    <?php else: ?>
                        <option value="0">Nenhuma</option>
                    <?php endif; ?>

                    <?php foreach ($familias as $familia): ?>  <!-- Existencia familia -->
                        <?php if($produto['id_familia'] == $familia['id_familia']): ?>
                            <option value="<?php echo $familia['id_familia']?>" selected><?php echo $familia['designacao'] ?></option> <!-- selected familia -->
                        <?php else: ?>
            ...
            <!-- imagem -- obs= testar carregamento ajax -->
            <div class="form-group card bg-light p-4">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-7 col-12">
                        ...
                        <input type="file" name="file_imagem" accept=".jpg, .png, .JPG" class="form-control">
                    </div>
                </div>                
            </div>
            ...
            <div class="row form-group align-items-center">
                ...
                <div class="col-3">
                    <select name="combo_taxa" class="form-control">
                        ...
                        <?php if($produto['id_taxa'] == 0): ?> <!-- Não existencia taxa -->
                            <option value="0" selected >Nenhuma (0 %)</option>
                        <?php else: ?>
                            <option value="0">Nenhuma (0 %)</option>
                        <?php endif; ?>

                        <?php foreach ($taxas as $taxa): ?> <!-- existencia taxa -->
            ...
            <div class="form-group text-center">
                ...
                <button class="btn btn-primary btn-150">Atualizar</button>  <!-- submissão -->
            </div>
