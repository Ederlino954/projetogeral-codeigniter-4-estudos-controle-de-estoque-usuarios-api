        ...
        <form action="<?php echo site_url('stocks/familia_editar/' . $familia['id_familia'])?>" method="post">
            ...
            <div class="form-group">
                ...
                <select name="select_parent" class="form-control">
                    ...
                    <?php 
                        $familia_nenhuma = '';
                        if($familia['id_parent'] == 0){ // $familia id base de comparação
                            $familia_nenhuma = "selected";
                        }
                    ?>                
                    <option value="0" <?php echo $familia_nenhuma ?>>Nenhuma</option>
                    <?php foreach ($familias as $fTemp): // $familia iria dar erro por causa da iteração ?>
                        <?php 
                            $familia_parent = '';
                            if($fTemp['id_familia'] == $familia['id_parent']) { // familia == parent
                                $familia_parent = 'selected'; /// selecionando caso exista 
                            }
                        ?>
                        <?php if($familia['id_familia'] != $fTemp['id_familia']): // diferenciando seleções na edição para não duplicar opções  ?>
