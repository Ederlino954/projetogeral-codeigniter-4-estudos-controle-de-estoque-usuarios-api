<?php 
    ...
    $s = session(); /// session dentro do controller users
?>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <?php foreach($users as $user): ?>
                    <tr>
                        <!-- editar e eliminar id_user da função setSession controller users  -->
                        <?php if($s->id_user == $user['id_user']): ?>  <!-- verificando o usuário da conta em tempo real -->
                            <td>
                                <span class="btn btn-secondary btn-sm"><i class="fa fa-pencil"></i></span> <!-- sem iperligação para não edição própria -->
                        ...
                        <?php else: ?>
                            <td>
                                ...
                                <?php if($user['deleted'] == 0):?>  <!-- Ativo ou inativo  -->
                                    <a href="<?php echo site_url('users/admin_delete_user/'.$user['id_user']) ?>" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
                                <?php else: ?>
                                    <a href="<?php echo site_url('users/admin_recover_user/'.$user['id_user']) ?>" class="btn btn-danger bg-success btn-sm"><i class="fa fa-recycle"></i></a>
                                <?php endif;?>
                        ...
                        <?php if(preg_match("/admin/", $user['profile'])):?> <!-- se admin dentro de profile  -->                         
                        ...
                        <!-- ativo ou inativo -->
                        <?php //if($user['active'] == 1): teste abaixo deu certo - / testar outros modos  ?>
