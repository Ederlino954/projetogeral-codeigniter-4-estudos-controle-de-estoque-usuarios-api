<?php
    ...
    $results = $gestor->EXE_QUERY( // select produtos 
    ...
    $produto = $results[0]; // consigo obter o id da taxa para cálculo 

    // Whithout taxes  // cálculo sem taxa
    $preco_total = $data['quantidade']*$produto['preco']; 
    $preco_total_sem_taxas = $preco_total; // usado ao fim do programa 
    $preco_total_com_taxas = $preco_total; // usado ao fim do programa 

    if ($produto['produto_id_taxa'] != 0) {
        // Whit taxes // cálculo com taxa 
        $preco_total = $preco_total * (1 + ($produto['percentagem']/100));
        $preco_total_com_taxas = $preco_total; // aqui ele diferencia 
    }  
    ...
    $params = array(
        ...
        'entrada_saida' => 'saída', // valor literal 
        'observacoes' => $data['observacoes']
    );

    // inserir movimentos no stock_movimentos // 0 autoincremento 
    ...
    // updade do stock produtos (Atualizar o stock de produto vendido)
    $params = array(
        ':id_produto' => $data['id_produto'],
        ':quantidade' => $data['quantidade'], // Subtração em quantidade 
    );
    $gestor->EXE_NON_QUERY("
    ...
    $response['RESULTS'] = array(
        ...
        'quantidaed disponivel' => $dTemp[0]['quantidade'],
