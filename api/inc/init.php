<?php
    ...
    // ---------------------------------------------
    // token
    if(array_key_exists('Token',$data)){ // case sensitive // Token do input
        $token = $data['Token'];
    } else {
    ...
    // ---------------------------------------------
    // check if app_key exists // chave para acesso 
    if(!key_exists('app_key', $data)){
        $response['STATUS'] = 'KO'; /// icorreto 
        ...
        if(count($dTemp) == 0){ // sem reusultados na query
            $response['STATUS'] = 'KO';
            ...
            $data['id_app'] = $dTemp[0]['id_app']; // pegando o app_id usando no remove_from_stock
        }
    }    